...
function onDataLoaded(dObj) {
    ...
    // prepare to nest data
    var qDirStep = 10
    var qDirScale = d3.scale.quantize()
        .domain([qDirStep,360])
        .range(d3.range(0, 360, qDirStep));
    var dirScale = d3.scale.linear() // value -> display
        .domain([0,360])
    ...
    for (var dir in nestedData) {
        ...
        for (var spd in nestedData[dir].values) {
            ...
            obj.windDir = [windDir,windDir+qDirStep]
            obj.windSpd = [windSpd,windSpd+qSpdStep]
            
            // setup data for arcs
            obj.startAngle = dirScale(obj.windDir[0])
            obj.endAngle = dirScale(obj.windDir[1])
            obj.innerRadius = spdScale(obj.windSpd[0])
            obj.outerRadius = spdScale(obj.windSpd[1])
    ...
    ctrdGrp.append("g")
        .selectAll("g")
            .data(nestedData)
            .enter().append("g")
                //.attr("transform", function(d1) { return "rotate(" + d1.key + ")"; })
                .selectAll("path")

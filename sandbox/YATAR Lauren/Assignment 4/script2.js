...
function onDataLoaded(dObj) {
    ...
    board.g.selectAll("rect")
        ...
        .attr({
            class: "bar",
            x: function(d) { if(d.mean < 0) { return (board.dDims.width / 2) - Math.abs((xMap(d)/2));}
            else return (board.dDims.width / 2);},
            y: function(d, i) { return i * (board.dDims.height / dObj.length); },
            height: '.75'*(board.dDims.width / dObj.length - barPadding),
        ...
        // For side numbers
         ...
         board.g.selectAll("text")
            ...
            .attr("x", 320)
            .attr("y", function(d, i) { return (board.dDims.height / dObj.length) - 5;})
      ...
      d3.select('svg').append('text').attr('x', 20).attr('y', 25).text('Average Degrees from Comfort, Averaged Hourly').style('font-family', 'Geneva').style('font-size', 11);
